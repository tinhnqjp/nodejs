(function () {
  ...
  var module = angular.module('autocheck.services', []);
  module.factory('AutoCheckService', function () {
    ...
    this.processProperties = function (field, valueInput, listMasterProperties) {
      ...
      var property_type = getTypeProperty(field.name, listMasterProperties);
      // AND with properties
    ...
    function getTypeProperty(field_name, listMasterProperties) {
      var field = {
        bukken: 0,
        deuta1: 0,
        deuta2: 0
      };
      var _dataField = field_name.split('_');
      ...
      if (field.bukken > 0 && field.deuta1 > 0) {
        var tmpPropertiesKo = _.filter(listMasterProperties, function (item) {
          return item.bukken === field.bukken &&
            item.daikoumoku === field.deuta1 &&
            item.kokoumoku === field.deuta2;
        });
        if (tmpPropertiesKo) {
          return tmpPropertiesKo[0].type;
        }
        return null;
      }
      return null;
